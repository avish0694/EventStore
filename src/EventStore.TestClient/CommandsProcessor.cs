...
namespace EventStore.TestClient
{
    public class CommandsProcessor
    {
        ...
        public bool TryProcess(CommandProcessorContext context, string[] args, out int exitCode)
        {
            ...
            {
                _log.Info("Unknown command: {@commandName}.", commandName);
            ...
            ThreadPool.QueueUserWorkItem(_ =>
            {
                try
                {
                    ...
                    {
                        exitC = 1;
                        _log.Info("Usage of {@commandName}:{@newLine}{@usage}", commandName, Environment.NewLine, commandProcessor.Usage);
                    }
                    executedEvent.Set();
                }
                catch (Exception exc)
                {
                    _log.ErrorException(exc, "Error while executing command {@commandName}.", commandName);
                    exitC = -1;
                    executedEvent.Set();
                }
            });
