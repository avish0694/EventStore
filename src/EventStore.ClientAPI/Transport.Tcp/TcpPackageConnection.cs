...
// ReSharper disable ImplicitlyCapturedClosure
            _connection = Connector.ConnectTo(
                ...
                (conn, error) =>
                {
                    ...
                    log.Debug("TcpPackageConnection: connection to [{0}, L{@localEndPoint}, {@connectionId:B}] failed. Error: {@error}.", conn.RemoteEndPoint, conn.LocalEndPoint, connectionId, error);
                },
                (conn, error) =>
                {
                    connectionCreated.Wait();
