...
namespace EventStore.Core
{
    public class ClusterNodeMutex
    {
        ...
        public bool Acquire()
        {
            ...
            catch (AbandonedMutexException exc)
            {
                Log.InfoException(exc,
                                  "Cluster Node mutex '{@mutexName}' is said to be abandoned. "
        ...
        public static bool IsPresent(int pid)
        {
            ...
            {
                Log.TraceException(exc, "Exception while trying to open Cluster Node mutex '{@mutexName}': {@message}.", mutexName, exc.Message);
            }
            return false;
        }
    }
