            ...
            private BloomFilter PopulateBloomFilter(){
                ...
                while(p < 1.0){
                    try{
                        ...
                        //Log.Debug("Created bloom filter with {@bf.NumBits} bits and {@bf.NumHashFunctions} hash functions for chunk {@Chunk.FileName} with map count: {@mapCount}", bf.NumBits, bf.NumHashFunctions, Chunk.FileName, mapCount);
                        break;
                    }
                ...
                if(bf == null){
                    Log.Warn(String.Format("Could not create bloom filter for chunk: {@Chunk.FileName}, map count: {@mapCount}",Chunk.FileName, mapCount));
                    return null;
                }
                ...
                try
                {
                    ...
                    //Log.Debug("{@mapCount} items added to bloom filter for chunk {@Chunk.FileName}", mapCount, Chunk.FileName);
                    return bf;
                }
