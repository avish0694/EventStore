            ...
            private BloomFilter PopulateBloomFilter(){
                ...
                while(p < 1.0){
                    try{
                        ...
                        //Log.Debug("Created bloom filter with {@numBits} bits and {@numHashFunctions} hash functions for chunk {@fileName} with map count: {@mapCount}", bf.NumBits, bf.NumHashFunctions, Chunk.FileName, mapCount);
                        break;
                    }
                ...
                try
                {
                    ...
                    //Log.Debug("{@mapCount} items added to bloom filter for chunk {@fileName}", mapCount, Chunk.FileName);
                    return bf;
                }
