...
namespace EventStore.Core.Services.PersistentSubscription
{
    public class PersistentSubscriptionCheckpointWriter : IPersistentSubscriptionCheckpointWriter
    {
        ...
        {
            Log.Debug("Publishing checkpoint for {@subscriptionStateStream}: {@state}", _subscriptionStateStream, state);
        ...
        private void WriteStateCompleted(ClientMessage.WriteEventsCompleted msg)
        {
            ...
            {
                Log.Debug("Checkpoint write successful for {@subscriptionStateStream}", _subscriptionStateStream);
                _version = msg.LastEventNumber;
            }
            else
            {
                Log.Debug("Error writing checkpoint for {@subscriptionStateStream}: {@result}", _subscriptionStateStream, msg.Result);
