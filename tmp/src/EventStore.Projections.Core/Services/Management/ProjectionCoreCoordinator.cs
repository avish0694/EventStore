...
namespace EventStore.Projections.Core.Services.Management
{
        ...
        IHandle<ProjectionCoreServiceMessage.SubComponentStopped>
    {
        ...
        private void StartWhenConditionsAreMet()
        {
            ...
            if (_systemReady && _ready)
            {
                if (!_started)
                {
                    ...
                    Log.Debug("PROJECTIONS: Starting Projections Core Coordinator. (Node State : {@_currentState})", _currentState);
                    Start();
                }
            }
            else
            {
                if (_started)
                {
                    Log.Debug("PROJECTIONS: Stopping Projections Core Coordinator. (Node State : {@_currentState})", _currentState);
                    Stop();
                }
            }
        }
        ...
        public void Handle(ProjectionCoreServiceMessage.SubComponentStarted message)
        {
            ...
            Log.Debug("PROJECTIONS: SubComponent Started: {@message.SubComponent}",message.SubComponent);
        ...
        {
            _activeSubComponents--;
            Log.Debug("PROJECTIONS: SubComponent Stopped: {@message.SubComponent}",message.SubComponent);
