    ...
    public class EmittedStreamsDeleter : IEmittedStreamsDeleter
    {
                ...
                onReadCompleted.Result == ReadStreamResult.NoStream)
            {
                ...
                if (onReadCompleted.Events.Length == 0)
                {
                    _ioDispatcher.DeleteStream(_emittedStreamsCheckpointStreamId, ExpectedVersion.Any, false, SystemAccount.Principal, x =>
                    {
                        ...
                        {
                            Log.Info("PROJECTIONS: Projection Stream '{@_emittedStreamsCheckpointStreamId}' deleted", _emittedStreamsCheckpointStreamId);
                        }
                        else
                        {
                            Log.Error("PROJECTIONS: Failed to delete projection stream '{@_emittedStreamsCheckpointStreamId}'. Reason: {@x.Result}", _emittedStreamsCheckpointStreamId, x.Result);
                        }
                        _ioDispatcher.DeleteStream(_emittedStreamsId, ExpectedVersion.Any, false, SystemAccount.Principal, y =>
                        {
                            ...
                            {
                                Log.Info("PROJECTIONS: Projection Stream '{@_emittedStreamsId}' deleted", _emittedStreamsId);
                            }
                            else
                            {
                                Log.Error("PROJECTIONS: Failed to delete projection stream '{@_emittedStreamsId}'. Reason: {@y.Result}", _emittedStreamsId, y.Result);
                            }
        ...
        private void DeleteStreamCompleted(ClientMessage.DeleteStreamCompleted deleteStreamCompleted, Action onEmittedStreamsDeleted, string streamId, long eventNumber)
        {
            ...
            else
            {
                ...
                Log.Error("PROJECTIONS: Failed to delete emitted stream {@streamId}, Retrying ({@(RetryLimit - _retryCount + 1}/{@RetryLimit}). Reason: {@deleteStreamCompleted.Result}", streamId, (RetryLimit - _retryCount) + 1, RetryLimit, deleteStreamCompleted.Result);
                _retryCount--;
